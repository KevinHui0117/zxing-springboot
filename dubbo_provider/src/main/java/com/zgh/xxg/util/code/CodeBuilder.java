...
public class CodeBuilder {
    ...
    private BufferedImage createCodeImage(CodeStrategy info) {
        String contents = info.getContents();
        int width = info.getWidth();
        int height = info.getHeight();
        Map<EncodeHintType, Object> hint = new HashMap<>();
        hint.put(EncodeHintType.ERROR_CORRECTION, info.getErrorCorrectionLevel());
        ...
        try {
            ...
            for (int x = 0; x < w; x++) {
                for (int y = 0; y < h; y++) {
                    img.setRGB(x, y, bm.get(x, y) ? info.getCodeColor() : info.getCodeBgdColor());
                }
            }
    ...
    public void createCodeImage(CodeStrategy info, OutputStream output) {
        BufferedImage bm = createCodeImage(info);
        File logoFile = info.getLogoFile();
        if (logoFile != null && logoFile.exists()) {
            try {
                BufferedImage logoImg = ImageIO.read(logoFile);
                int logoWidth = info.getLogoWidth();
                int logoHeight = info.getLogoHeight();
                ...
                if (!StringUtils.isEmpty(desc)) {
                    ...
                    Font font = fontUtil.loadFontResource(info.getFontSize());
                    ...
                    g1.setColor(new Color(info.getCodeFontColor()));
                    g1.setFont(font);
                    ...
                    for (int i = 0; i < desc.length(); i++) {
                        ...
                        if (currentLineLen + charWidth > width) {
                            currentLineIndex++;
                            currentLineLen = 0;
//                            g1.drawString(c, currentLineLen + whiteWidth + (width - whiteWidth - currentLineLen - charWidth) / 2, height + baseLo + fontHeight * (currentLineIndex) + wordTopMargin);
...
//                        g1.drawString(c, currentLineLen + whiteWidth + (width - whiteWidth - currentLineLen - charWidth) / 2, height + baseLo + fontHeight * (currentLineIndex) + wordTopMargin);
     ...
     * @param file
     */
    public void createCodeImage(CodeStrategy info, File file) {
    ...
    public void createCodeImage(CodeStrategy info, String filePath) {
