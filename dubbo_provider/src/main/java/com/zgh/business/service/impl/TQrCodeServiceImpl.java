...
import com.zgh.business.entity.*;
 ...
 */
@Service
public class TQrCodeServiceImpl implements QrCodeServiceI {
    @Resource
    ...
    public Map<String, Object> produceQrCode(Map<String, Object> req) {
        ...
        //生成二维码的渠道名称
        String genChannel = req.get("genChannel") == null ? "" : req.get("genChannel").toString();
        String codeName = genChannel + UUID.randomUUID().toString();
        tSysQrcode.setCodeName(codeName);
        if (req.get("effectiveDate") == null || req.get("expirationDate") == null) {
            response.put("code", "0");
            response.put("msg", "生效/失效日期不得为空，生成失败，请重试！");
            return response;
        } else {
            if (req.get("effectiveDate").toString().length() < 19 || req.get("expirationDate").toString().length() < 19) {
                response.put("code", "0");
                response.put("msg", "生效/失效日期格式为 yyyy-MM-dd HH:mm:ss，生成失败，请重试！");
                return response;
            }
        }
        ResponseBean _saveRes = saveOrUpdate(operater, tSysQrcode);
        ...
        Map<String, Object> result = generateCode(operater, tSysQrcode);
    ...
    private Map<String, Object> generateCode(Operater operater, TSysQrcode tSysQrcode) {
        return this.generateQrCode(tSysQrcode, operater);
    }

    @Async("saveOrUpdate")
    public ResponseBean saveOrUpdate(Operater operater, TSysQrcode tSysQrcode) {
        return this.saveOrUpdate(tSysQrcode, operater);
    }
...
//                calendar.setTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(effectiveDate + " 00:00:00"));
                calendar.setTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(effectiveDate));
...
//                calendar.setTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(expirationDate + " 23:59:59"));
                calendar.setTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(expirationDate));
